...
// C. Functions called to perform custom checks by anon function in section A
...
// -------------------------------------------------------------------------
  $('form').on('submit', function (e) {           // When form is submitted
    ...
    var valid = {};                             // Custom valid objects
    ...
    for (i = 0, l = elements.length; i < l; i++) {
        // Next line calls validateRequired() and validateTypes()
    ...
    if (!validateBio()) {                       // Call validateBio(), and if not valid
        ...
        removeErrorMessage(document.getElementById('bio')); // Remove error message
    }

    // password (you could cache password input in variable here) - See password-signup.js that has replaced this validation
    ...
    // parental consent (you could cache parents-consent in variable here)
    if(!validateParentsConsent()) {             // Call validateParentsConsent(), and if not valid
        ...
        valid.parentsConsent = false;           // Update valid object - this element is not valid
    } else {                                    // Otherwise
    ...
    for ( var field in valid) {                 // Check properties of the valid object
        ...
        isFormValid = true;                     // The form is valid and okay to submit
    }
...
function validateRequired(el) {             
    if (isRequired(el)){                         // Is this element required?
        ...
        return valid;                            // Return valid varable, true or false
    }                                       
...
function validateTypes(el) {
    ...
    if (typeof validateType[type] === 'function') { // Is the type a method of validate object
        return validateType[type](el);            // If yes, checks if the value validates
    } else {                                      // If not,
...
/*** Check that the passwords both match and are 8 characters or more - Commented out because another stronger password validation
  is included in password-signup.js ***/
/** function validatePassword() {
    var password = document.getElementById('password');
    var valid = password.value.length >= 8;
    if(!valid) {
        setErrorMessage(password, 'Please make sure your password has atleast 8 characters');
    }
    return valid;
}   **/
...
function removeErrorMessage(el) {
    ...
    errorContainer.remove();                        // Remove the error message
}
...
var validateType = {
    email: function(el) {                           // Create email() method
        ...
        return valid;                               // Return valid variables
    },
    ...
    //password: function(el) {  /*** To check a password between 8 to 15 characters which contain 
       // at least one lowercase letter, one uppercase letter, one numeric digit, and one special character ***/

       //var valid =  /^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[^a-zA-Z0-9])(?!.*\s).{8,15}$/.test(el.value);
      // if(!valid) { 
       //setErrorMessage(el, 'Please enter a valid password which must be between 8 to 15 characters and contain at least one lowercase letter, one uppercase letter, one numeric digit, and one special character');
